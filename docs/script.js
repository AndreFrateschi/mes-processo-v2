(function () {
  ...
  if (page === "detail") {
    ...
    $("#objective-preliminary")?.closest(".check-field")?.remove();
    ...
    function openEditor(item) { const isNew = !item; const objective = item || { id: "", name: "", description: "", status: "planned", progress: 0, deliveries: [], nextStep: "", owner: "A definir" }; $("#editor-stage").textContent = stage.shortTitle; $("#editor-title").textContent = isNew ? "Criar novo objetivo" : "Editar objetivo"; $("#objective-id").value = objective.id; $("#objective-name").value = objective.name; $("#objective-description").value = objective.description; $("#objective-status").value = objective.status; $("#objective-progress").value = objective.progress; $("#objective-deliveries").value = objective.deliveries.join(", "); $("#objective-next-step").value = objective.nextStep; $("#objective-owner").value = objective.owner; $("#delete-objective").hidden = isNew; $("#duplicate-objective").hidden = isNew; $("#editor-message").textContent = ""; editor.showModal(); }
    function formObjective(id) { return { id, name: $("#objective-name").value.trim(), description: $("#objective-description").value.trim(), status: $("#objective-status").value, progress: Math.max(0, Math.min(100, Number($("#objective-progress").value) || 0)), deliveries: $("#objective-deliveries").value.split(",").map(v => v.trim()).filter(Boolean), nextStep: $("#objective-next-step").value.trim(), owner: $("#objective-owner").value.trim() || "A definir" }; }
    ...
    $("#close-editor").addEventListener("click", () => editor.close()); $("#new-objective").addEventListener("click", () => openEditor());
