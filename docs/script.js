(function () {
  ...
  if (page === "detail") {
    ...
    function openEditor(item) { const isNew = !item; const objective = item || { id: "", name: "", description: "", status: "planned", progress: 0, deliveries: [], nextStep: "", owner: "A definir", preliminary: true }; $("#editor-stage").textContent = stage.shortTitle; $("#editor-title").textContent = isNew ? "Criar novo objetivo" : "Editar objetivo"; $("#objective-id").value = objective.id; $("#objective-name").value = objective.name; $("#objective-description").value = objective.description; $("#objective-status").value = objective.status; $("#objective-progress").value = objective.progress; $("#objective-deliveries").value = objective.deliveries.join(", "); $("#objective-next-step").value = objective.nextStep; $("#objective-owner").value = objective.owner; $("#objective-preliminary").checked = objective.preliminary; $("#delete-objective").hidden = isNew; $("#duplicate-objective").hidden = isNew; $("#editor-message").textContent = ""; editor.showModal(); }
